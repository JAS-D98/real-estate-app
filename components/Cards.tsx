  ...
  return (
    <TouchableOpacity onPress={onPress} className="flex flex-col items-start w-60 h-72 relative">
      ...
      <View className='flex flex-col items-start absolute bottom-5 inset-x-5'>
        <Text className='text-md font-rubik-extrabold text-white' numberOfLines={1}>{name}</Text>
...
export const Card = ({item:{image, rating, name, address, price}, onPress}:Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex-1 w-full mt-4 py-4 px-2 rounded-lg bg-white shadow shadow-black-100/70 relative'>
      ...
      <View className='flex flex-col mt-2'>
        ...
        <Text className='text-xs font-rubik text-white'>{address}</Text>
